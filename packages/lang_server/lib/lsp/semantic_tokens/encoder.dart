...
class SemanticTokenEncoder {
  ...
  SemanticTokens encodeTokens(
    ...
    for (final token in sortedTokens) {
      ...
      final tokenLine = location.line + _serverToLspLineOffset;
      final tokenColumn = location.column + _serverToLspLineOffset;
  ...
  Iterable<SemanticTokenInfo> splitMultilineTokens(
    ...
    for (var line = start.line; line <= end.line; line++) {
      final isFirstLine = line == start.line;
      final isLastLine = line == end.line;
      final lineOffset = lineInfo.getOffsetOfLine(line - 1);

      final startOffset = isFirstLine ? start.column - 1 : 0;
      final endOffset = isLastLine
          ? end.column - 1
          : lineInfo.getOffsetOfLine(line) - lineOffset;
