...
class Interpreter implements ExprVisitor, StmtVisitor {
  ...
  void init({
    ...
    try {
      _sdkLoc = sdkLocation ?? 'assets/hetu/';
      this.workingDir = workingDir ?? 'assets/script';
      ...
      // 载入基础库
      await evalf(_sdkLoc + 'core.ht', displayLoadingInfo: displayLoadingInfo);
      await evalf(_sdkLoc + 'value.ht', displayLoadingInfo: displayLoadingInfo);
      await evalf(_sdkLoc + 'system.ht', displayLoadingInfo: displayLoadingInfo);
      await evalf(_sdkLoc + 'console.ht', displayLoadingInfo: displayLoadingInfo);
    } catch (e) {
  ...
  void evalf(String filepath,
    ...
    if (!_evaledFiles.contains(curFileName)) {
      ...
      var content = await rootBundle.loadString(_curFileName);
      eval(content, curFileName, context: library_namespace, style: style, invokeFunc: invokeFunc, args: args);
    }
    _curFileName = null;
  }
  ...
  dynamic visitImportStmt(ImportStmt stmt) {
    ...
    if (stmt.location.startsWith('hetu:')) {
      file_loc = _sdkLoc + stmt.location.substring(5) + '.ht';
    } else {
      file_loc = workingDir + stmt.location;
    }
    evalf(file_loc, libName: stmt.nameSpace);
  }
