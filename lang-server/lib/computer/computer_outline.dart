...
class HetuModuleOutlineComputer {
  ...
  Outline compute() {
    ...
    for (final node in parseResult.nodes) {
      if (node is ClassDecl) {
        ...
        outlines.add(_newFunctionOutline(functionDeclaration, true));
      } else if (node is ClassTypeAlias) {
        var alias = node;
        outlines.add(_newClassTypeAlias(alias));
      } else if (node is FunctionTypeAlias) {
        var alias = node;
        outlines.add(_newFunctionTypeAliasOutline(alias));
      } else if (node is GenericTypeAlias) {
        var alias = node;
        outlines.add(_newGenericTypeAliasOutline(alias));
      }
    }
    var unitOutline = _newUnitOutline(outlines);
    return unitOutline;
  }
  ...
  Location _getLocationOffsetLength(int offset, int length) {
    ...
    var startLocation = parseResult.lineInfo.getLocation(offset);
    var startLine = startLocation.lineNumber;
    var startColumn = startLocation.columnNumber;
    var endLocation = parseResult.lineInfo.getLocation(offset + length);
  ...
  List<Outline> _outlinesForMembers(List<AstNode> members) {
